...
if(str_detect(string = cur.directory, pattern = "Nikki")){
  data.dir.path <- "/Users/Nikki/Dropbox/Consulting Project/Data/"
}
...
# # One attempt
# nonnum.covars.to.include <- clti %>% group_by(primaryProcedure) %>% 
#   select(all_of(comorbidity.var.names), primaryProcedure) %>%
#   summarise_at(comorbidity.var.names, sum) %>%
#   mutate_at(comorbidity.var.names, 
#             function(col){col * (1/349)}) %>%
#   pivot_longer(cols = anyAnemia:CAD, 
#                names_to = "comorbidity", 
#                values_to = "proportion") %>%
#   pivot_wider(id_cols = comorbidity, 
#               names_from = primaryProcedure, 
#               values_from = proportion) %>%
#   filter(Revasc >= 0.05 & WM >= 0.05) %>%
#   select(comorbidity)
# 
# nonnum.covars.to.include <- nonnum.covars.to.include$comorbidity
...
# covar.names <- c("ageAtIndexProcedure", nonnum.covars.to.include,
                 # disease.var.names, "GENDER", "nonWhite", "ETHNICITY")
covar.names <- c("GENDER", "InflowDisease", "OutflowDisease", "RunoffDisease",
...
results.sum.df$Outcome <- c("Amp or Death", "Death", "Amputation", "MACE", "Amp or Death", "Death", "Amputation", "MACE")
...
aim2.plot <- ggplot(data = results.sum.df, aes(x = Outcome, y = RD.hat,  ymin = Lower, ymax = Upper))+
