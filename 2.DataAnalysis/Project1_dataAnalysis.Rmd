...
```{r prevalence_summary}
clti %>% select(-all_of(names(clti.binary.as.logical))) %>% table1(x = ~. | primaryProcedure, data = .)
```
...
ampsurv.aipw.boot <- boot(data = clti, statistic = calcAIPWate, R = 500, 
                          outcome.name = "ampMort2yr", 
                          cov.names = covar.names, lambda.to.use = "lambda.1se")  
```


```{r aim2_mort_boot, cache=TRUE, eval = FALSE}
mort.aipw.boot <- boot(data = clti, statistic = calcAIPWate, R = 100, 
                          outcome.name = "mort2yr", 
                          cov.names = covar.names, lambda.to.use = "lambda.1se")  
```


```{r aim2_amp_boot, cache=TRUE, eval=FALSE}
amp.aipw.boot <- boot(data = clti, statistic = calcAIPWate, R = 100, 
                          outcome.name = "majorAmp2yr", 
                          cov.names = covar.names, lambda.to.use = "lambda.1se")  
```


```{r aim2_mace_boot, cache=TRUE, eval = FALSE}
mace.aipw.boot <- boot(data = clti, statistic = calcAIPWate, R = 100, 
...
results.sum.df$Outcome <- rep(c("Amp or Death", "Death", "Amputation", "MACE"), times = 3)[1:9]
results.sum.df$Method <- c(rep("Unadjusted", 4), rep("CTMLE", 4), rep("AIPW",1))
